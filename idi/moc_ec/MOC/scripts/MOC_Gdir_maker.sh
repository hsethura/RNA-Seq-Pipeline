...
CISID_PATH=`config_read $CONFIG_FILE gdriveCIS_path`
...
### Make local directory
	...
	if [ $TYPE == "DP" ] || [ $TYPE == "TCP" ] || [ $TYPE == "CP" ] ;then
	...
	if [ $TYPE == "TCP" ] || [ $TYPE == "CP" ];then
		if [ $TYPE == "TCP" ];then
			GLOCAL_DIR=$GLOCAL_PATH"/"$GCID_PATH"/"$PROC"/"
		fi
		if [ $TYPE == "CP" ];then
			GLOCAL_DIR=$GLOCAL_PATH"/"$CISID_PATH"/"$PROC"/"
		fi		
	...
	if [ $TYPE == "DE" ] || [ $TYPE == "TCE" ] || [ $TYPE == "CE" ] || [ $TYPE == "P" ];then
		...
		if [ $TYPE == "CE" ];then
			GLOCAL_DIR=$GLOCAL_PATH"/"$CISID_PATH"/"$PROC"/Experiments/"$ID
		fi
	...
	if [ $TYPE == "P" ];then
		...
		if [ $NUM_ID == 1 ];then
			...
			echo $NUM_ID "entries in $PC_DB found for "$ID
			exit
		fi
...
# # 	yes Y | $GDRIVE_SCRIPT share -quiet -with-link -role writer $KEY_NAME 
	...
	echo "Changing permissions on $GLOCAL_DIR $TEMP_DIR"
	change_perms $GLOCAL_DIR 
	ls -lrt $TEMP_DIR
