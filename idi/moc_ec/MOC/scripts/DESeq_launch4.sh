...
DE_PATH=`config_read $CONFIG_FILE DE_path`
...
DS_DIR=$TEMP_PATH"/"$ID"/"$PROJ_ID"/DESeq_temp/"
OUT_DIR=$DE_PATH"/"$ID"/"$PROJ_ID"/DE/DESeq2/"
TEMP_DIR=$TEMP_PATH"/"$ID"/"$PROJ_ID"/DESeq2/"
...
READ_FILE=$TEMP_DIR"/"$PROJ_ID"=REP_Genes_"$ACC"_Reads.txt"
DS1_RFILE=$TEMP_DIR$PROJ_ID"-"$TAG"_DS1.R"
...
tag2="_"$numA"vs"$numB
DS1_RESULTS=$TEMP_DIR"DESeq1_results_"$PROJ_ID"-"$OUTPUT_TAG".txt"
DS2_RESULTS=$TEMP_DIR"DESeq2_results_"$PROJ_ID"-"$OUTPUT_TAG".txt"
DS1_OUTPUT=$TEMP_DIR"DESeq1_out"$PROJ_ID"-"$OUTPUT_TAG".txt"
DS2_OUTPUT=$TEMP_DIR"DESeq2_out_"$PROJ_ID"-"$OUTPUT_TAG".txt"
DS1_MA=$TEMP_DIR"DESeq1_"$PROJ_ID"-"$OUTPUT_TAG"_MA.pdf"
DS2_MA=$TEMP_DIR"DESeq2_"$PROJ_ID"-"$OUTPUT_TAG"_MA.pdf"
...
DESeq2R_maker $READ_OUTPUT $KEY_OUTPUT $DS2_RESULTS $DS2_MA $OUTPUT_TAG $test > $DS2_RFILE 
...
cp $DS2_RESULTS $DS2_MA $DS_STATS $OUT_DIR
ls -lrt $OUT_DIR*
echo $OUT_DIR