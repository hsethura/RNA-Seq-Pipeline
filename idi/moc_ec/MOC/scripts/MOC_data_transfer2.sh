...
do
	...
	if [ ! -s $RESULTS_PATH ];then
 	...
 	GRES_DIR_ADDRESS=$GDRIVE_ADDRESS$GID_RES
...
#################
	...
	echo "Email for data receipt: "$DATA_EMAIL > $MESSAGE_FILE
	echo ""  >> $MESSAGE_FILE
	echo "Dear Collaborator," >> $MESSAGE_FILE
	echo ""  >> $MESSAGE_FILE
	echo "The analysis of "$MOC_ID" "$PROJ_ID" has been completed." >> $MESSAGE_FILE
	echo ""  >> $MESSAGE_FILE
	echo "All count and metrics files along with results from differential expression analysis have been uploaded to Google Drive at the following URL:" >> $MESSAGE_FILE
 	echo $GRES_DIR_ADDRESS >> $MESSAGE_FILE 
	echo ""  >> $MESSAGE_FILE	
		
	echo "For EXTERNAL collaborators (those with access to the Broad server):">> $MESSAGE_FILE
	echo ""  >> $MESSAGE_FILE
	echo "We are in the process of setting up an Aspera site from which you can get retrieve your results and raw data files.">> $MESSAGE_FILE
	echo "You will be receiving an email describing how to get your data from this site shortly.">> $MESSAGE_FILE
	echo ""  >> $MESSAGE_FILE

	echo "For INTERNAL collaborators (those with access to the Broad server):">> $MESSAGE_FILE
	echo ""  >> $MESSAGE_FILE
	...
	cat $MESSAGE_FILE |  mailx -s "Analysis for $MOC_ID $PROJ_ID complete" $USID"@broadinstitute.org"
