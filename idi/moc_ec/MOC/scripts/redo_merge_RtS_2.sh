...
if [ $COMBINE == "Y" ];then
	...
	ALL_IDS=`ls -lrt $PREMERGE_DIR | awk '{print $9}'| grep fastq | sed 's/R[0-9].fastq.gz//g'| sed -e 's/_add[0-9]_/_/g' | sort | uniq`
	...
	do
		...
		do
			...
			echo $ID "ID"
			ALL_ADD_FASTQS=`ls -lrt $PREMERGE_DIR/* | grep $ID | grep "_add" | grep $READ | grep fastq.gz | awk '{print $NF}'`
			NUM_ADD_FASTQS=`echo $ALL_ADD_FASTQS | wc -w`
			ALL_FASTQS=$FIRST_FASTQ" "$ALL_ADD_FASTQS
			...
			if [ $NUM_ADD_FASTQS -gt 0 ];then
				...
				ls -l $FIRST_FASTQ $ALL_ADD_FASTQS 
				ls -lrt $OUT_FASTQ
