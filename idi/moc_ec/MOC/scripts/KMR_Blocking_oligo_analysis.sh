...
CDS_find $FILE b3316 b3316.txt


ALL_IDS=`cat b3316.txt | head -1 | awk '{
										for(i=2; i < NF+1; i++)
											print $i
									}'`
...
do
	...
	total_b3316=`total b3316.txt $ID`
	ratio=`echo $total_16S $total_b3316 | awk '{print $1/$2}'`
	echo $ratio
	#echo $total_16S"\t"$total_b3316"\t"$ratio
	
	ratio=`echo $total_23S $total_b3316 | awk '{print $1/$2}'`
        echo $ratio
        #echo $total_23S"\t"$total_b3316"\t"$ratio
done
