...
try:
    ...
    parser.add_argument('--Suffix', '-S', type = str, required = True, default = 'none', help = 'Key_dir to store downloaded key files')
...
def main():
    ...
    if flags.Key_dir == 'none':
        KeyDir = "/broad/IDP-Dx_storage/MOC/PoolLogs"
    else:
        KeyDir = flags.Key_dir


    outfile = KeyDir + "/" + proid  + Suff
    outf = open(outfile, 'w')
