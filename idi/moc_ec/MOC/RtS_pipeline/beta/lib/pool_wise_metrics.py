# This file summarizes the sample-wise metrics at a pool-level
...
def pool_wise_metrics(config, join_metrics_fpath):
    # sample-wise data frame
    df_sample = pd.read_csv(join_metrics_fpath, sep='\t')
    ...
    pool_wise_metrics_fpath = join_metrics_fpath.replace('.txt', '_poolwise.csv')
    df_pool.to_csv(pool_wise_metrics_fpath, index=False)

    print ('Finished processing')
    print ('Pool-wise metrics written to: {}'.format(pool_wise_metrics_fpath))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--moc_id', type = str, required = True, help ='MOC_ID, eg: MOCP-0120')
    options = parser.parse_args()

    config = yaml.load(open(options.config_file))
    join_metrics_dir = os.path.join(config['join_path'], options.moc_id)

    for fname in os.listdir(join_metrics_dir):
        # Find pool-wise metric file in the directory
        if fname.endswith('_KeyMetrics.txt'):
            join_metrics_fpath = os.path.join(join_metrics_dir, fname)
            print ('Generating pool-wise metrics by parsing: {}'.format(join_metrics_fpath))

            pool_wise_metrics(config, join_metrics_fpath)
