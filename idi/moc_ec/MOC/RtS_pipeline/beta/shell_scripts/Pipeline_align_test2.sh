...
BASE=`echo $FASTQ_FILE | awk '{y=split($1, ar, "/"); print ar[y]}' | sed -e 's/.gz//g' -e 's/.fastq//g'`
ROOT=$BASE"_"$STRAIN
...
do
	...
	do
		if [ -s $NCBI_PATH$m".fna" ];then
			...
			echo "Aligning $sampled_fastq1 to generate $ALN_SAM"
			echo "sh $SCRIPT_DIR"FASTQ_align.sh" $sampled_fastq1 $NCBI_PATH$m".fna" $TEMP_DIR$ROOT"_"$m"_mapped_"$num_reads"
			sh $SCRIPT_DIR"FASTQ_align.sh" $sampled_fastq1 $NCBI_PATH$m".fna" $ALN_SAM_ROOT
