...
parser$add_argument("-k", "--key_path", dest="key_path", type="character", 
    default = "junk", help="Path to the key file")
parser$add_argument("-d", "--d_type", dest="d_type", type="character", 
    default = "t", help="Delimeter for the input file")
...
count_tab1 <- read.csv(count_path, sep = sep_str, header = TRUE, row.names = 1, 
    stringsAsFactors = FALSE, check.names = FALSE)

rnames1 <- rownames(count_tab1)
rnames <- rnames1[grep("^CDS", rnames1)]
count_tab <- count_tab1[rnames, ]
