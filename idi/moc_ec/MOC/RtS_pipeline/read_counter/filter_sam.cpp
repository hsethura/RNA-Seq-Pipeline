...
bool filter_samc::parse_args(int argc, char* argv[]) {
    bool all_set = true;
...
filter_samc::flagvals filter_samc::get_flagvals(int flag) {
...
std::string filter_samc::get_format(std::string lfname_str, std::string open_mode) {
...
htsFile* filter_samc::get_hts_handle(std::string lfname_str, std::string format) {
    htsFile* lfp = NULL;
...
void filter_samc::bam_handle_init(std::string fname_str, const bam_hdr_t* lhdr,
        htsFile*& lfp_out, bam_hdr_t*& lhdr_out, std::string open_mode) {
...
bool filter_samc::has_suffix(const std::string &str, const std::string &suffix) {
    return str.size() >= suffix.size() &&
           str.compare(str.size() - suffix.size(), suffix.size(), suffix) == 0;
}


bool filter_samc::check_write(bam_hdr_t *lhdr, bam1_t *lread1, bam1_t *lread2) {
...
void filter_samc::start_rts_paired() {
