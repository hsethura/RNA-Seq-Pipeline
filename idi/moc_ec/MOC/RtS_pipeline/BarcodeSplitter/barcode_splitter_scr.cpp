...
void bc_splitter::print_help() {
    std::cout << desc << "\n";
	std::cout << "Usage: bc_splitter_scr -d <dict_file> --file1 <file1> --file2 <file2>"
...
bool bc_splitter::parse_args(int argc, char* argv[]) {
    ...
    // First 8bp is UMI. We want the next 13bp which is the well-barcode
	barcode_start = 8;
	barcode_size = 13;
...
bc_splitter::updateMaps(std::string& barcode_str, 
	...
	// Trim lword2 and lword4. Trim the first 21 bases (UMI + WB)

	int lword2_len = lword2_ori.length();
	int lword4_len = lword4_ori.length();
	int lword2_lefts = lword2_len - barcode_start - barcode_size;
	int lword4_lefts = lword4_len - barcode_start - barcode_size;

	std::string lword2 = lword2_ori.substr(barcode_start+barcode_size, lword2_lefts);
	std::string lword4 = lword4_ori.substr(barcode_start+barcode_size, lword4_lefts);
...
void bc_splitter::split_engine() {
	...
	while (file1.getline(lword1)) {
		...
		// We want 13 bases starting fro the 9th base (first 8bp is UMI)
		...
		std::string smallest_barcode = "JJJJJJJJJJJJJ";
